    ...
    def update_subnational_boundaries(self, country, levels, do_not_process):
        ...
        for level in levels:
            ...
            if len(boundary_resource) == 0:
                logger.warning(f"{iso}: Could not find boundary resource")
                continue
            ...
            boundary_lyr["ADM0_REF"] = country["#country+name+preferred"]

            fields = boundary_lyr.columns
            for l in range(1, int(level[-1]) + 1):
                possible_pcode_fields = [field.replace("#", l) for field in self.headers["pcode"]]
                possible_name_fields = [field.replace("#", l) for field in self.headers["name"]]
                pcode_field = None
                name_field = None
                ...
                for field in fields:
                    if not pcode_field:
                        if field.upper() in possible_pcode_fields:
                            pcode_field = field
                    if not name_field:
                        if field.upper() in possible_name_fields:
                            name_field = field

                if not name_field:
                    logger.error(f"{iso}: Could not map name field for adm{l}")
                    continue
            ...
            self.boundaries[f"{level}_polygon"] = self.boundaries[f"{level}_polygon"][
                self.boundaries[f"{level}_polygon"]["alpha_3"] != iso
            ]
            self.boundaries[f"{level}_polygon"] = self.boundaries[f"{level}_polygon"].append(boundary_lyr)
            self.boundaries[f"{level}_polygon"].sort_values(by=[f"ADM{level[-1]}_PCODE"], inplace=True)
            logger.info(f"{iso}: Finished processing {level} boundaries")

            # convert polygon boundaries to point
            points = GeoDataFrame(self.boundaries[f"{level}_point"].representative_point())
            points.rename(columns={0: "geometry"}, inplace=True)
            points[req_fields] = self.boundaries[f"{level}_point"][req_fields]
            points = points.set_geometry("geometry")
            self.boundaries[f"{level}_point"] = points
